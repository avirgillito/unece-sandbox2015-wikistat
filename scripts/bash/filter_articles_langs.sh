...
proj1=\"<proj> <file_with_articles> <input_for_2012_2015>\"
proj2=\"<proj> <file_with_articles> <input_for_2012_2015>\"
...
" 1>&2; exit 1; }
...
existsdir() { echo $1 " exists. This directory should not exist." 1>&2; exit 1; }
...
#    $cmd &
    cmd="hadoop jar /usr/hdp/2.3.2.0-2950/hadoop-mapreduce/hadoop-streaming.jar -D mapred.reduce.tasks=7 -file ${mapper} -file ${artfile} -mapper ${mapper} -file ${reducer} -reducer ${reducer} -input ${input} -output ${tempdir}/"$proj"_old -cmdenv WIKI_PROJ=${proj} -cmdenv WIKI_AGGR=${aggregation} -cmdenv ART_FILE=$(basename ${artfile}) -cmdenv FIRST_YEAR=2012 -cmdenv FIRST_MONTH=1 -cmdenv FIRST_DAY=1 -cmdenv LAST_YEAR=2015 -cmdenv LAST_MONTH=12 -cmdenv LAST_DAY=31"
    $cmd &
done
echo "waiting.."
wait
